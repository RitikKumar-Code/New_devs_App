...
export const RevenueSummary: React.FC<RevenueSummaryProps> = ({ propertyId = 'prop-001', debugTenant, showRaw }) => {
    ...
    const totalValue = Number(data.total_revenue);
    const displayTotal = Number.isFinite(totalValue)
        ? totalValue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })
        : data.total_revenue;
    const reportingPeriod = new Date(Date.UTC(data.period_year, data.period_month - 1, 1)).toLocaleString(undefined, {
        month: 'long',
        year: 'numeric',
        timeZone: 'UTC'
    });
            ...
            <div className="p-6">
                <div className="flex items-center justify-between mb-6">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-gray-500">{reportingPeriod} in {data.timezone}</p>
                ...
                <div className="mt-4 h-6">
                    {showRaw && (
                        <div className="flex items-center text-xs text-emerald-700 bg-emerald-50 px-2 py-1 rounded">
                            Revenue is rendered from an exact server-formatted money value.
                        </div>
                    )}
                </div>
            </div>
